...
class CleanPage(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        ctk.CTkButton(self.toolbar, text=t("btn.select_all"), width=130,
                      command=lambda: self.toggle_all(True)).pack(side="left")
        ctk.CTkButton(self.toolbar, text=t("btn.none"), width=90,
                      command=lambda: self.toggle_all(False)).pack(side="left", padx=8)
        ctk.CTkButton(self.toolbar, text=t("btn.preview"), width=110,
                      command=self.app.preview_clean).pack(side="left")
        ctk.CTkButton(self.toolbar, text=t("btn.winapp_rules"), width=130,
                      command=self.app.load_winapp_rules).pack(side="left", padx=8)
        self.clean_btn = ctk.CTkButton(self.toolbar, text=t("btn.clean_selected"),
                                       width=170,
        ...
        self.total_lbl = ctk.CTkLabel(self.total_frame,
                                      text=t("clean.total_calculating"),
                                      font=fluent_font(13, "bold"))
        self.total_lbl.pack(side="left", padx=12, pady=6)
        self.progress = ctk.CTkProgressBar(self.total_frame, height=10,
                                           mode="determinate",
                                           progress_color=accent)
        self.progress.set(0)
        self.progress.pack(side="left", fill="x", expand=True, padx=12, pady=6)
        self.status_lbl = ctk.CTkLabel(self.total_frame, text=t("status.ready"),
                                       text_color=MUTED)
    ...
    def _build_rows(self):
        ...
        for cat in self.app.categories:
            ...
            ctk.CTkLabel(row, text=cat.description + admin,
                         font=fluent_font(11), text_color=MUTED).grid(row=1, column=2, sticky="w")

            right = ctk.CTkFrame(row, fg_color="transparent")
            right.grid(row=0, column=3, rowspan=2, padx=10, sticky="e")
            size_lbl = ctk.CTkLabel(right, text=t("label.calculating"),
