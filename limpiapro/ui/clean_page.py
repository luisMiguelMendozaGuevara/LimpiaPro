...
class CleanPage(ctk.CTkFrame):
    def __init__(self, master, app):
        ...
        page_header(self, "Limpieza del sistema",
                    "Selecciona lo que quieres limpiar. Nada se elimina sin tu confirmacion.")
        ...
        self.status_lbl = ctk.CTkLabel(self.total_frame, text="Listo", text_color=MUTED)
    ...
    def update_after_scan(self, cat):
        if cat.recycle_bin:
            # El tamano ya lo calculo el worker de analisis (app._analyze_worker).
            txt = f"~{format_size(cat.size)}" if cat.size else "papelera vacia"
